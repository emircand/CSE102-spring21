...
void test_operate_polynomials()
{
	...
	printf("------%s-----\n","Polynomial");
	printf("enter first polynomial ((3, a), (2, b), (1, c), (0, d)):\n");
	scanf("(3, %lf), (2, %lf), (1, %lf), (0, %lf)", &a3, &a2, &a1, &a0);
	getchar();
	printf("enter second polynomial ((3, a), (2, b), (1, c), (0, d)):\n");
	scanf("(3, %lf), (2, %lf), (1, %lf), (0, %lf)", &b3, &b2, &b1, &b0);
	getchar();
	printf("choose an operator: (sum '+' subtract '-' multiplication '*'): ");
	scanf("%c",&op);
	getchar();
