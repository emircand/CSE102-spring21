...
struct node_list* merge_list(struct node_list* head_1, struct node_list* head_2)
{
    ...
    /*reach NULL pointer at the end*/
    while (temp->next->next->next != NULL)
    {
        temp = temp->next;
    }
    /*after reaching NULL pointer, connect two nodes*/
    if(temp->next->next->next == NULL)
        temp->next->next->next = head_2;
...
struct node_list* merge_interleaved(struct node_list* head_1, struct node_list* head_2)
{
    ...
    struct node_list* h1 = head_1, *h2 = head_2;
    ...
    if (h1 != NULL && h2 != NULL)
    {
        ...
        temp = &(*h2);
        temp = temp->next;
        merge_interleaved(h1->next, h2->next);
    }
    return temp;
}
